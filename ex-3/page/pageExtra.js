...
/*
  ...
  */
  ...
  _findPanelPath:function(e,ps,os){
    let i=os.indexOf(e),a=os[i-1],as=[],_start;
    $("*").toArray().find(x=>{
      if(x==e){
        return 1
      }else if(x==a){
    ...
    as=as.filter(a=>!_Util._isIgnoreElement(a)&&(a.innerText||"").trim()&&(!ee.w||!(a.innerText||"").includes(ee.w))&&a.getBoundingClientRect().top<=ee.top+20)
    as=as.map((x,i)=>{
      return {
        w:(x.innerText||"").trim(),
        o:x,
    ...
    aa=as[0]
    let pe=_Util._getShareParent(aa.o,e),r=["BZ.TW.document",`${pe.tagName}:Contains(${aa.w})`,...ps]
    os=$util.findDoms(r)
    i=os.indexOf(e)
  ...
  _findPath:function(e,_withAIAnalysis,_simple){
    try{
      ...
      let _definitedPath=_cssHandler._getComPath(e,_withAIAnalysis)
      if(_definitedPath){
        return _definitedPath
      }
      ...
      }else if(!_cssHandler._isInShadowDom(e)&&e.tagName!="CANVAS"){
        let _path=_findSimplePath(e)
        if(_path){
          return _path
        }
      }
  ...
  //      console.time("_findPath")
    ...
    function _findSimplePath(e){
      let p=e.tagName,os;
      if(e.id&&!_cssHandler._isAutoValue(e.id)){
        p+="#"+e.id
        if($(p)[0]==e){
      ...
      if(!_isInput(e)){
        ...
        return _cssHandler._findInputPath(e)
      }
      let cp=_cssHandler._getUniqueClass(e)
      if(cp){
        if(os){
          os=os.filter(cp)
          p+=cp
      ...
      if(os[0]==e){
        ...
        return _cssHandler._findPanelPath(e,[p],os.toArray())
      }
    }
  },
  _isAutoValue:function(v){
    return v&&v.match(/[0-9]{2,}/i)
  },
  _isIgnoreClass:function(c){
    return c.match(_cssHandler._ignoreClasses)||_cssHandler._isAutoValue(c)
  },
  ...
  _getUniqueClass:function(e){
    ...
    for(var i=0;i<e.classList.length;i++){
      ...
      if(_cssHandler._isPriorityElementItem(m)){
        ...
        if(!_cssHandler._isIgnoreClass(m)){
          cs.push(m)
        }
      }
    };
...
const $script={
  ...
  exe:function(actions,_fun){
    ...
    function _doIt(actions){
      let a=actions.shift()
      if(a){
        bzComm._log("$script action: ")
        bzComm._log(a.log?a.log.replace("##Action## ",""):a.constructor.name+" "+a.element)
        _results.push(a)
        if(bzComm._isIDE()){
          let t=_IDE._getEnvAdvanceSetting().stepDelay
          let aa=_ideTask._buildTaskAction(a)
          BZ._data._status="play"
          _ideTask._action._doTest(aa,0,(v)=>{
            if(v=="_bzSkip"){
              a._result={_bzSkip:1,_type:4,_url:v._url};
            }else{
              a._result={_type:v._type,_msg:v._msg,_url:v._url};
            }
            if(v._type<3){
              return _end()
            }
    ...
    function _end(){
      if(!_playing&&bzComm._isIDE()){
        _ideTask._end()
      }
      let r=_results._last()
      if(r){
        r=r._result
      }
      if(r){
        _fun&&_fun({_type:r._type,_msg:r._msg,_url:r._url,_results:_results})
      }else{
        _fun&&_fun()
      }
    }
